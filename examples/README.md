...
| --- | --- | --- |
| [`bare/send.php`](bare/send.php) | one client, one of each call, then close | you are evaluating the SDK or checking a credential |
| [`basic/app.php`](basic/app.php) | an HTTP server that instruments itself | you are wiring the SDK into a real service |
...
every stack trace, so the `new Intempt(['apiKey' => …])` frame carries the key
into every later exception. No SDK can fix that from the inside.
...
Batching is deliberately off here. Under PHP-FPM a process handles one request
and dies, so an in-memory buffer has nowhere to live — turn batching on only in a
worker, a queue consumer, Swoole or RoadRunner.
...
The point is the shape, not the routes: one client per request, an identifier on
every call, and every failure surfaced rather than swallowed.
...
```bash
INTEMPT_HOST=127.0.0.1:8931 INTEMPT_SCHEME=http php examples/bare/send.php
```
